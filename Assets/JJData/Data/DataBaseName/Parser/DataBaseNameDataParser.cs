...
public static class DataBaseNameDataParser
{
    ...
    public static void SetXlsxData()
    {
        ...
        ConvertRowData2Sheet1();
    }
    public static void ConvertRowData2Sheet1()
    {
        int stringLength = 0;
        TextAsset binAsset = Resources.Load<TextAsset>(Path.Combine(_dataFilePath, "DataBaseName/Sheet1"));
        Span<byte> buffer = binAsset.bytes;
        int offset = 0;
        long rows = TypeByte2TypeConverter.ConvertTypeByte2long(buffer.Slice(offset, 8));
        offset += 8;
        DataBaseName.Sheet1 = new DataBaseName_Sheet1[rows];
        for(int row = 0; row < rows; row++)
        {
            DataBaseName_Sheet1 sheetRowData = new();
            ...
            offset += stringLength;
            DataBaseName.Sheet1[row] = sheetRowData;
        }
    ...
    {
        int stringLength = 0;
        TextAsset binAsset = Resources.Load<TextAsset>(Path.Combine(_dataFilePath, "DataBaseName/TestClass"));
        Span<byte> buffer = binAsset.bytes;
        int offset = 0;
        long rows = TypeByte2TypeConverter.ConvertTypeByte2long(buffer.Slice(offset, 8));
        offset += 8;
